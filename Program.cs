...
//Diccionario de libros agrupados por categoria
var diccionarioLookup = queries.DiccionariosDeLibrosPorAñoDePublicacion();
ImprimirDiccionarioString(diccionarioLookup, "2012");
...
void ImprimirDiccionarioString(ILookup<string, Book> ListadeLibros, string filter)
{
    Console.WriteLine("{0,-60} {1, 15} {2, 15}\n", "Titulo", "N. Paginas", "Fecha publicacion");
    foreach (var item in ListadeLibros[filter])
    {
        Console.WriteLine("{0,-60} {1, 15} {2, 15}", item.Title, item.PageCount, item.PublishedDate.Date.ToShortDateString());
    }
}